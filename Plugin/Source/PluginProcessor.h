...
public:
    ...
    {
        editorWidth.store (w);
        editorHeight.store (h);
        if (editorResizeFn) editorResizeFn (w, h);
    }
    int getSavedEditorWidth() const noexcept  { return editorWidth.load(); }
    int getSavedEditorHeight() const noexcept { return editorHeight.load(); }
...
private:
    ...
    void handleFinishHostStemExport     (const juce::var&, juce::WebBrowserComponent::NativeFunctionCompletion);
    ...
    std::atomic<int> editorWidth  { 300 };
    std::atomic<int> editorHeight { 500 };
