...
export default function CalendarPanel({ events, categories, currentUserId, groupTitleById, onDelete, onSetCategory }: Props) {
                    ...
                    <div className="cal-ev-main">
                      ...
                      {editing === e.id && (
                        <div className="cal-catpick">
                          ...
                          <input className="cal-catpick-new" placeholder="New category…" onKeyDown={(ev) => { if (ev.key === 'Enter') { const v = (ev.target as HTMLInputElement).value.trim(); if (v) { onSetCategory(e.id, v); setEditing(null) } } }} />
                        </div>
                      )}
                    </div>
                    {own && (
