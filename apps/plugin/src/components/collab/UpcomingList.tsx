...
/** The programme margin, top-left of the orbit home — split at a glance
 *  into TODAY (times only, full ink) and NEXT (the following few days).
 *  Same column as the app's home page, sized for the plugin. Tapping any
 *  line opens the calendar. */
export default function UpcomingList({ events, onOpen }: Props) {
  const { todayRows, nextRows } = useMemo(() => {
    ...
    return {
      todayRows: coming.filter((e) => new Date(e.starts_at) < dayEnd).slice(0, 3),
      nextRows: coming.filter((e) => new Date(e.starts_at) >= dayEnd).slice(0, 3),
    }
  }, [events])
