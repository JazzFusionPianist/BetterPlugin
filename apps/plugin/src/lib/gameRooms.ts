...
export type GameType = 'chess' | 'falling_blocks' | 'poker' | 'ear_training' | 'yacht' | 'sketch'
...
const DEFAULT_PLAYER_COUNT: Record<GameType, number> = {
  chess: 2, ear_training: 2, falling_blocks: 4, poker: 6, yacht: 4, sketch: 6,
}
...
export async function createGameRoom (
  ...
  if (gameType === 'falling_blocks' || gameType === 'yacht' || gameType === 'sketch') {
    const table = GAME_TABLE[gameType]
    ...
    if (gameType === 'yacht' || gameType === 'sketch') insert.state = {}
    const { data, error } = await supabase
      .from(table)
      .insert(insert)
      .select()
...
export async function findActiveGame (
  ...
  const [chess, fb, poker, et, yacht, sketch] = await Promise.all([
  ...
  const sketchRoom = sketch.data?.find(isResumableMultiPlayerRoom)
  ...
  if (sketchRoom) candidates.push({ gameType: 'sketch', roomId: sketchRoom.id, updatedAt: sketchRoom.updated_at })
