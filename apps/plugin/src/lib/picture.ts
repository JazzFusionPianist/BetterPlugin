/*  The picture domain — how the wall's background is made.
    ...
    · `plot` zips what lands on x, y, z — sample by sample, from the
      ...
      is one value, so it is one point. Each axis has a min and a max,
      in the unit that arrives: that range spans the wall edge to edge.
...
export const PIC_MEMORY = 36, PIC_INDEX = 37, PIC_DECIBEL = 38
export const isPictureType = (t: number) => t === FX_PLOT || t === PIC_MEMORY || t === PIC_INDEX || t === PIC_DECIBEL
...
export function plotPoints (spec: PictureSpec, plot: PicNode, rings: Rings, maxBuckets = 2048): { pts: Float32Array; n: number } {
  ...
  const range = (a: number) => { const mn = (plot.aux[a * 2] ?? -100) / 100, mx = (plot.aux[a * 2 + 1] ?? 100) / 100; return { mn, d: mx - mn } }
  const R = [range(0), range(1), range(2)]
  const unit = (a: number, v: number) => (R[a].d === 0 ? 0 : Math.max(-8, Math.min(8, (v - R[a].mn) / R[a].d * 2 - 1)))
  let n = 0
  for (let j = 0; j < L; j++) {
