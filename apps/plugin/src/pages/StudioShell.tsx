...
function StudioShellInner({ supabase, user }: Props) {
  ...
  // mergeDroppedRegions came back empty for that mode — the row greys
  // out with a note instead of failing silently. 'placed' usually means
  // the stamps can't prove one timeline; 'joined' means nothing decoded.
  const [mergeFailed, setMergeFailed] = useState<{ placed: boolean; joined: boolean }>({ placed: false, joined: false })
  ...
  useEffect(() => {
    ...
    setDropChoice(null); setDropBusy(null); setMergeFailed({ placed: false, joined: false })
  }, [activeConvId])
  ...
  const openDropChoice = useCallback((choice: PendingDropChoice) => {
    setDropZip(false)
    setDropBusy(null); setMergeFailed({ placed: false, joined: false })
  ...
  const choiceMerge = useCallback((mode: 'placed' | 'joined') => {
    ...
    setDropBusy(mode)
    void (async () => {
      const merged = await mergeDroppedRegions(c.files, mode)
      setDropBusy(null)
      if (!merged) { setMergeFailed(f => ({ ...f, [mode]: true })); return }   // chooser stays up, row greys
      setDropChoice(null)
      if (c.target === 'stems') {
        setPendingStemDrop({
          id: `choice-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
            ...
            <div className="wd-dropask" role="dialog" aria-modal="true">
              ...
              <div className="wd-dropask-card">
                ...
                >
                  ...
                  <small>{mergeFailed.placed ? 'no timing info in these files' : 'as placed on the session timeline'}</small>
                </button>
                <button
                ...
                >
                  ...
                  <small>{mergeFailed.joined ? 'these files can’t be merged' : 'gaps removed, one continuous take'}</small>
                </button>
                <button
