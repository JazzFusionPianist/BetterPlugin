...
window.__JUCE__ = {
  initialisationData: { __juce__functions: ['trackExportHost', 'trackExport'], __juce__platform: ['mac'] },
  backend: {
    addEventListener: (_, fn) => { listeners.add(fn) },
    removeEventListener: (_, fn) => { listeners.delete(fn) },
    emitEvent: (_, value) => {
      const { name, params, resultId } = value as { name: string; params: string[]; resultId: number }
      if (name !== 'trackExportHost' && variant === 'loading') return
      const result = name === 'trackExportHost' ? 'Standalone' : JSON.stringify(
        params[0] !== 'inspectTracks' ? { ok: false, error: 'QA only: no audio was exported or sent.' }
        ...
        : { ok: true, sessionId: 'qa', name: 'Midnight Harbour — Full Band', sampleRate: 48000,
          ...
          ranges: { entire: variant === 'midi' ? null : { start: 0, end: 9600000 }, selection: { start: 48000, end: 480000 } },
